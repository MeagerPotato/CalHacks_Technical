...
const STATUS_FILL_CLASSES = {
  draft: "bg-page",
  submitted: "bg-accent",
  in_review: "bg-highlight",
  accepted: "bg-success",
  waitlisted: "bg-action",
} as const;
          ...
          {types.map((entry) => (
            <div key={entry.type} data-type={entry.type} className="flex flex-col gap-3">
              ...
              <ul role="list" className="flex flex-col gap-3">
                {entry.rows.map((row) => (
                  <li key={row.status} data-status={row.status} className="flex flex-col gap-1 rounded-control p-1 hover:bg-page">
                    <div className="flex flex-wrap items-baseline justify-between gap-x-3">
                      <span className="font-semibold">{row.label}</span>
                      <span className="text-sm">{row.countText}</span>
                    </div>
                    <div aria-hidden="true" className="h-3 w-full overflow-hidden rounded-full border-2 border-border bg-page">
                      <div className={`h-full ${STATUS_FILL_CLASSES[row.status]}`} style={{ width: `${row.percent}%` }} />
                    </div>
                  </li>
                ))}
              </ul>
