...
import { formatCalendarDate, toTimestampView } from "@/lib/format/datetime";
...
function countryName(code: string | null): string | null {
  return COUNTRY_OPTIONS.find((option) => option.value === code)?.label ?? null;
}

/**
 * Identifying details for a revealed workspace, in About you order. This is the only organizer view-model code that
 * reads `ApplicantIdentity` fields:
 * - the name prefers the full-name answer, then the profile display name;
 * - the birthdate is formatted as a date, and the country code becomes its name;
 * - the affiliation label depends on the type;
 * - links are kept only when they are http(s) URLs, and open in a new tab.
 */
    ...
    fields: [
      {
        key: "name",
        label: copy.name,
        text: presentText(identity.fullName) ?? presentText(identity.displayName),
        links: [],
      },
      { key: "email", label: copy.email, text: presentText(identity.email), links: [] },
      { key: "birthdate", label: copy.birthdate, text: formatCalendarDate(identity.birthdate), links: [] },
      {
        key: "countryOfResidence",
        label: copy.countryOfResidence,
        text: countryName(identity.countryOfResidence),
        links: [],
      },
      { key: "cityOfResidence", label: copy.cityOfResidence, text: presentText(identity.cityOfResidence), links: [] },
