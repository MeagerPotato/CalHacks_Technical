...
describe("toIdentityView", () => {
  it("reads the name, email, affiliation, and http links", () => {
  ...
  it("labels a Judge affiliation and falls back to the display name", () => {
    const view = toIdentityView("judge", { ...applicantIdentity("judge"), preferredName: " ", affiliation: null });
    expect(view.fields[0].text).toBe("Ada Builder");
    expect(view.fields[2]).toEqual({ key: "affiliation", label: "Company / organization", text: null, links: [] });
    expect(view.fields[3].links).toEqual([]);
  });
  ...
  it("drops links that are not http or https URLs", () => {
    ...
    expect(view.fields[3].links).toEqual([{ label: "HTTPS://Example.com/Profile", href: "HTTPS://Example.com/Profile" }]);
  });
});
...
describe("toReviewWorkspaceView", () => {
  ...
  it("shows identity only when the data layer returned it", () => {
    ...
    expect(view.identity?.fields.map((field) => field.key)).toEqual(["name", "email", "affiliation", "links"]);
  });
