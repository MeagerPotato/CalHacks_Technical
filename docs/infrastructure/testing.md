...
| File | Covers |
|---|---|
| `auth.spec.ts` | Signup offers only Hacker and Judge checkboxes, rejects a tampered Organizer value, and explains a missing choice. Also: signup through onboarding to the portal dashboard, a wrong password, `next` redirects with an unsafe-`next` fallback, and sign-out. |
...
| `schedule.spec.ts` | The landing page timeline lists the four stops in order with the configured dates, with at most one current stop. With Playwright's clock, the countdowns switch to the browser clock after hydration and tick every second. |
