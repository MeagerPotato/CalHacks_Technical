...
- **Confirm email:** off in the hosted Supabase project since 2026-09-11. Signups receive a session immediately and no email is sent, so no custom SMTP is configured. Email addresses are therefore unverified: create the Organizer account yourself and promote it at once (see [Hosted Supabase](#hosted-supabase), step 4). `/auth/callback` still handles confirmation links if it is turned back on.
...
3. **Configure Auth in the dashboard.** `supabase/config.toml` holds local values, so do not run `supabase config push` without reviewing `supabase config diff` first. Set:
   ...
   - **Authentication → Sign In / Providers → Email:** keep email signup enabled. **Confirm email** is off for this deployment, so `signUp` returns a session with `requiresEmailConfirmation: false` and the applicant lands on `/onboarding` without an email. Turning it on (the hosted default) restores the check-your-email notice and the `/auth/callback` link, and then requires custom SMTP.
   - **Custom SMTP (Authentication → Emails → SMTP Settings):** needed only while **Confirm email** is on. This deployment has both off. With confirmation on, configure a provider such as Resend, Postmark, or Amazon SES before sharing the link: Supabase's built-in sender delivers only to members of the project's organization (other addresses fail with "Email address not authorized") and sends only a few emails per hour, so applicants could not confirm their accounts. The sender address must sit on a domain verified with that provider, including the exact subdomain — on 2026-09-11 a sender on `allenkh.com` while Resend had verified `mail.allenkh.com` failed every signup with a 550 and the applicant saw "Something went wrong". Once custom SMTP is on, Supabase starts at 30 emails per hour; raise the email limit under **Authentication → Rate Limits** to cover the expected signups.
   - **Why confirmation is off here:** a demo deployment needs anyone to reach the portal without waiting on email. The cost is unverified addresses, so create the Organizer account before the URL is shared (step 4), and never promote an account that already existed.
