...
- **Confirm email:** on in the hosted Supabase project, with custom SMTP through Resend and the sender on `mail.allenkh.com`, the domain verified with Resend. A signup emails a confirmation link that completes at `/auth/callback` in the same browser. Verified end to end in production on 2026-09-11.
...
3. **Configure Auth in the dashboard.** `supabase/config.toml` holds local values, so do not run `supabase config push` without reviewing `supabase config diff` first. Set:
   ...
   - **Custom SMTP (Authentication → Emails → SMTP Settings):** configure it before sharing the link, with a provider such as Resend, Postmark, or Amazon SES. Without it, Supabase's built-in sender delivers only to members of the project's organization (other addresses fail with "Email address not authorized") and sends only a few emails per hour, so applicants could not confirm their accounts. This deployment uses Resend.
     - **The sender address must sit on the exact domain verified with the provider.** On 2026-09-11 a sender on `allenkh.com`, while Resend had verified only the `mail.allenkh.com` subdomain, failed every signup: Supabase logged `550 "The allenkh.com domain is not verified"` and returned 500, and the applicant saw "Something went wrong". Moving the sender to `mail.allenkh.com` fixed it.
     - Once custom SMTP is on, Supabase starts at 30 emails per hour; raise the email limit under **Authentication → Rate Limits** to cover the expected signups.
     - Supabase also throttles repeat sends to one address: a second signup for the same email within seconds returns 429 `over_email_send_rate_limit`.
   - **Fallback if SMTP cannot be ready in time:** turn **Confirm email** off. Applicants are then signed in immediately, but email addresses are not verified, so create the Organizer account before the URL is shared (step 4), and never promote an account that already existed.
